...
class Square:
    ...
    def draw(self, screen):
        if self.is_hovering:
            pygame.draw.rect(screen, pygame.Color(2,12,30), (self.x - 3, self.y - 3, self.size + 6, self.size + 6))
            pygame.draw.rect(screen, HOVER_COLOR, (self.x, self.y, self.size, self.size))
        else:
            pygame.draw.rect(screen, DEFAULT_COLOR, (self.x, self.y, self.size, self.size))
